...
window.boot = function () {
    ...
    var onStart = function () {
        ...
        let subCount = 0, loadCount = 0;;
        for(let index in json.subpackages){
            const subpack = json.subpackages[index]
            loadCount ++;
            const loadTask = wx.loadSubpackage({
                name: subpack.name, // name 可以填 name 或者 root
                success: function(res) {
                // 分包加载成功后通过 success 回调
                    subCount++;
                    if(subCount >= loadCount){
            ...
            loadTask.onProgressUpdate(res => {
                    setLoading(parseInt((res.totalBytesWritten / res.totalBytesExpectedToWrite) * 100) + "%")
